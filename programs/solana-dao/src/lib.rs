...
declare_id!("HQNsBk3UiLkQQEDr9qEBs3PC1AhPNeYGfh1WyNhLW5Tx");
...
pub mod ayes_vs_nays {
	...
	pub fn vote_ayes(ctx: Context<Vote>) -> ProgramResult {
    	let vote_account = &mut ctx.accounts.vote_account;
    	vote_account.ayes += 1;
    	Ok(())
	}
	pub fn vote_nays(ctx: Context<Vote>) -> ProgramResult {
    	let vote_account = &mut ctx.accounts.vote_account;
    	vote_account.nays += 1;
    	Ok(())
	}
...
/// We define a struct with two public properties: ayes and nays
